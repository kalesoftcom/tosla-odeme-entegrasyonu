...
class Gateway
{
    ...
    public function post($url, $postdata = [])
    {
        ...
        // Ensure standard curl execution
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            'Expect:' // Disable 100-continue behavior
        ]);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

        // Connection fixes for local environments
    ...
    public function validateHash($data)
    {
        if (isset($data['HashParameters'])) {
            ...
            foreach ($keys as $key) {
                // Check if key exists in extra, then in data. If nowhere, empty string.
                if (isset($extra[$key])) {
                    $hashString .= $extra[$key];
                }
